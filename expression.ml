...
let rec expr_to_java exp = match exp with
   ...
   | LitS (str) -> "\"" ^ str ^ "\""
   | Exists (str1, str2) -> "//TODO: match type and perform correct lookup characters.containsKey(\"" ^ str1 ^ "\") || locations.containsKey(\"" ^ str2 ^ "\")"
...
let rec expr_to_java_boolean exp = ([], "true")
